...
public static class ArchiveResourceIndexBuilder
{
    public static ArchiveConflictSummary[] Build(IReadOnlyList<ResourceProvider> resources, IReadOnlyList<Mo2Archive> archives, IReadOnlyList<string> archiveOrder, IReadOnlyList<RdarArchiveFailure>? failures = null, bool archiveSetIncomplete = false)
    {
        ...
        archiveSetIncomplete |= failures is { Count: > 0 };
        ...
        foreach (IGrouping<ulong, ResourceProvider> hashGroup in resources.GroupBy(value => value.ResourceHash))
        {
            ...
            {
                Add(buckets, providers[0], archiveSetIncomplete ? ArchiveResourceDisposition.Unresolved : ArchiveResourceDisposition.Unique, archiveSetIncomplete ? null : providers[0].ArchiveName, [], archiveSetIncomplete ? ArchivePayloadRelation.Unknown : ArchivePayloadRelation.NotApplicable);
                continue;
            }
            ...
            bool evidenceMissing = providers.Any(value => !positions.ContainsKey(value.ArchiveName));
            ...
            string?[] winnerPayloads = winnerGroup.Select(value => value.PayloadFingerprint).ToArray();
            ...
            string? winnerPayload = unresolved ? null : winnerPayloads[0];
            for (int index = 0; index < ordered.Length; index++)
            {
                ...
                bool matchesWinner = index > 0 && winnerPayload is not null && string.Equals(provider.PayloadFingerprint, winnerPayload, StringComparison.OrdinalIgnoreCase);
                ...
                ArchivePayloadRelation relation = matchesWinner || belongsToAmbiguousWinner && !winnerPayloadUnknown ? ArchivePayloadRelation.Identical
                    : unresolved || provider.PayloadFingerprint is null || winnerPayload is null ? ArchivePayloadRelation.Unknown
                    : index == 0 ? ArchivePayloadRelation.NotApplicable
                    : ArchivePayloadRelation.Different;
                Add(buckets, provider, disposition, winner, archiveNames.Where(value => !string.Equals(value, provider.ArchiveName, StringComparison.OrdinalIgnoreCase)).ToArray(), relation);
            }
        }
    ...
    private static void Add(Dictionary<string, Buckets> buckets, ResourceProvider provider, ArchiveResourceDisposition disposition, string? winner, string[] otherArchives, ArchivePayloadRelation relation)
    {
        ...
        ArchiveResourceOutcome outcome = new(provider.ResourceHash, display, disposition, winner, provider.PayloadFingerprint, provider.ResourceType, provider.PathConfidence, otherArchives, relation);
