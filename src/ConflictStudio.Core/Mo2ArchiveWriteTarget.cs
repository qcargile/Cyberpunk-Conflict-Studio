...
public sealed record Mo2ArchiveWriteTarget(string ModlistPath, string Provider = "Overwrite", ModManagerKind ManagerKind = ModManagerKind.Mo2, string? ContextPath = null, string? WriteBlockedReason = null, string? ExpectedContextId = null, string? ExpectedProfileId = null);
...
public static class Mo2ArchiveWriteTargetResolver
{
    public static Mo2ArchiveWriteTarget Resolve(string mo2Root)
    {
        ...
        return new Mo2ArchiveWriteTarget(Path.Combine(paths.OverwriteRoot, "archive", "pc", "mod", "modlist.txt"), "Overwrite", ModManagerKind.Mo2, null, blocked);
    }
    ...
    public static Mo2ArchiveWriteTarget Resolve(string mo2Root, ArchiveOrderEvidence? evidence)
    {
        ...
        if (evidence is { Kind: ArchiveOrderEvidenceKind.ManagedModlist, SourcePath: not null } && File.Exists(evidence.SourcePath)) return new Mo2ArchiveWriteTarget(Path.GetFullPath(evidence.SourcePath), evidence.Provider ?? "Active provider", ModManagerKind.Mo2, null, fallback.WriteBlockedReason);
        return fallback;
    }
}
