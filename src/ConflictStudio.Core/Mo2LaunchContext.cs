...
public static class Mo2LaunchContextResolver
{
    ...
    {
        string? root = InstanceRoot(explicitRoot);
        if (root is not null) return Context(root, Mo2LaunchEvidence.ExplicitRoot);
    ...
    private static string? InstanceRoot(string? candidate)
    {
