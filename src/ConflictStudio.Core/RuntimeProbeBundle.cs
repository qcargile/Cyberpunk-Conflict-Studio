...
public sealed record RuntimeProbeReceipt(int SchemaVersion, string ProfileName, string? InstallationId, string RunId, string ManifestId, DateTimeOffset ImportedAtUtc, bool CompleteRun, RuntimeProbeObservation[] Observations);
...
public static class RuntimeProbeReceiptReader
{
    ...
    private static RuntimeProbeObservation Observation(RuntimeProbeBundleRequest request, Dictionary<string, Match> matches, IReadOnlyDictionary<string, string>? manualAnswers)
    {
        ...
        return state switch
        {
            ...
            "failed" => new RuntimeProbeObservation(request.Id, RuntimeProbeObservationState.Failed, null, value),
