...
{
    public const string Boundary = "Observed, failed, and manual probe records do not prove compatibility, compilation, loader success, a runtime defect, callback order, provider attribution, or a final winner.";
}
...
public static class RuntimeProbeBundleWriter
{
    ...
    private static string Instructions(RuntimeProbeBundleManifest manifest)
    {
        ...
        return $"Conflict Studio runtime probe for profile: {manifest.ProfileName}{Environment.NewLine}Manifest: {manifest.ManifestId}{Environment.NewLine}Run: {manifest.RunId}{Environment.NewLine}{Environment.NewLine}This bundle is opt-in and does not modify game data. Install the bin folder as a separate mod in the same manager profile, deploy or activate it, launch the matching profile, and remain at the main menu for at least 10 seconds before closing the game. The automated values are sampled once, five seconds after CET begins updating, after all onInit handlers have returned.{Environment.NewLine}{Environment.NewLine}Automated post-initialization values: {automated}{Environment.NewLine}Manual checks: {manual}{Environment.NewLine}{Environment.NewLine}Automated values are observed TweakDB facts only. They do not establish compatibility, compilation, loader success, or a runtime defect. Manual checks remain unresolved until an answer is recorded. The receipt carries no compiler or loader artifact, so it cannot decide those boundaries.{Environment.NewLine}{Environment.NewLine}For manual checks, copy manual-answers.example.json and replace each answer with the observed result. Keep that file with the ConflictStudioProbe log and casefile when sending the evidence to the maintainer. Repository maintainers can import those files with the source-built CLI. Remove or disable ConflictStudioProbe after collecting the evidence.{Environment.NewLine}";
    }
