...
public enum RedScriptOverlapKind { ExclusiveReplacement, CompositionReview, AddedMemberCollision, AddedMemberInteraction }

public sealed record RedScriptHook(string Provider, string FilePath, RedScriptHookKind Kind, string Target);
...
{
    private static readonly Regex Annotation = new("^\\s*@(?<kind>wrapMethod|replaceMethod|addMethod|addField)\\((?<class>[^)]+)\\)", RegexOptions.Compiled);
    ...
    public static RedScriptOverlap[] Analyze(IReadOnlyList<RedScriptSource> sources)
    {
        ...
        foreach (RedScriptSource source in RedScriptConditionalSourceFilter.Filter(sources))
        {
            ...
            RedScriptHookKind? currentKind = null;
            foreach (string line in code.Split('\n'))
            {
                Match annotation = Annotation.Match(line);
                if (annotation.Success)
                {
                    currentClass = annotation.Groups["class"].Value.Trim();
                    currentKind = annotation.Groups["kind"].Value switch
                    {
                ...
                if (currentClass is not null && currentKind is not null)
                {
                    Match declaration = currentKind == RedScriptHookKind.AddField ? Field.Match(line) : Method.Match(line);
                    if (declaration.Success)
                    {
                        string target = currentKind == RedScriptHookKind.AddField
                            ? currentClass + "." + declaration.Groups["field"].Value
                            : RedScriptTarget.Normalize(currentClass, declaration.Groups["method"].Value, declaration.Groups["parameters"].Value);
                        hooks.Add(new RedScriptHook(source.Provider, source.FilePath, currentKind.Value, target));
                        currentClass = null;
                        currentKind = null;
                    }
                }
    ...
    private static RedScriptOverlapKind Classify(IEnumerable<RedScriptHook> hooks)
    {
        ...
        return values.Count(value => value.Kind == RedScriptHookKind.Replace) > 1 ? RedScriptOverlapKind.ExclusiveReplacement : RedScriptOverlapKind.CompositionReview;
    }
}
