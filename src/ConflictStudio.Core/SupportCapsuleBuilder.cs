...
public static class SupportCapsuleBuilder
{
    public static SupportCapsule Build(ProfileScanReceipt receipt, IReadOnlyList<EvidenceDecision> decisions)
    {
        ...
        EvidenceDecision[] profileDecisions = decisions.Where(value => string.Equals(value.ProfileName, receipt.ProfileName, StringComparison.Ordinal) && string.Equals(value.InstallationId, receipt.InstallationId, StringComparison.Ordinal)).ToArray();
