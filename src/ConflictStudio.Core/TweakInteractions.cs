...
public static class TweakInteractionAnalyzer
{
    ...
    {
        foreach (TweakOperation copy in operations.Where(value => IsArrayCopy(value.Kind)))
        {
            TweakOperation[] sourceWrites = operations.Where(value => value.Target == copy.Value && !string.Equals(value.Provider, copy.Provider, StringComparison.OrdinalIgnoreCase)).ToArray();
            if (sourceWrites.Length == 0) continue;
            yield return new TweakOverlap(copy.Target + " <- " + copy.Value, TweakOverlapKind.SourceArrayDependency, [copy, .. sourceWrites]);
        }
    }
