...
public sealed class ArchiveXlManifestAnalyzerTests
{
    ...
    public void AnalyzeDetailedReportsMalformedAndUnsupportedManifests()
    {
        ...
        Assert.IsTrue(result.Failures.Any(value => value.Provider == "Broken" && value.Kind == ArchiveXlFailureKind.Malformed && value.Message.Contains("represented", StringComparison.OrdinalIgnoreCase)));
        Assert.IsTrue(result.Failures.Any(value => value.Provider == "Future" && value.Kind == ArchiveXlFailureKind.Coverage && value.Message.Contains("transform", StringComparison.Ordinal)));
    }
    ...
    [TestMethod]
    public void SourceScanExcludesAnUnboundWinnerWithoutFallingBackToALoser()
    {
        string root = Path.Combine(Path.GetTempPath(), "conflict-studio-archive-xl-excluded-" + Guid.NewGuid().ToString("N"));
        string first = Path.Combine(root, "First");
        string second = Path.Combine(root, "Second");
        try
        {
            WriteManifest(first, "shared.xl", "factories:\n  - first\\factory.csv\n");
            WriteManifest(second, "shared.xl", "factories:\n  - second\\factory.csv\n");
            WriteManifest(second, "other.xl", "factories:\n  - second\\other.csv\n");
            string excluded = Path.Combine(first, "archive", "pc", "mod", "shared.xl");

            ArchiveXlSourceScanResult scan = ArchiveXlSourceScanner.Scan([new ArchiveXlProviderSource("First", first), new ArchiveXlProviderSource("Second", second)], null, new HashSet<string>(StringComparer.OrdinalIgnoreCase) { excluded });

            Assert.AreEqual("archive\\pc\\mod\\other.xl", scan.Sources.Single().FilePath);
            Assert.IsFalse(scan.Sources.Any(value => value.FilePath == "archive\\pc\\mod\\shared.xl"));
        }
        finally
        {
            Directory.Delete(root, true);
    ...
    [TestMethod]
    public void SourceScanReservesTheIdentityOfADisappearedHighPriorityManifest()
    {
        string root = Path.Combine(Path.GetTempPath(), "conflict-studio-missing-archive-xl-" + Guid.NewGuid().ToString("N"));
        string high = Path.Combine(root, "High");
        string middle = Path.Combine(root, "Middle");
        string low = Path.Combine(root, "Low");
        try
        {
            WriteManifest(middle, "shared.xl", "factories:\n  - middle\\factory.csv\n");
            WriteManifest(low, "shared.xl", "factories:\n  - low\\factory.csv\n");
            WriteManifest(low, "other.xl", "factories:\n  - low\\other.csv\n");
            string disappeared = Path.Combine(high, "archive", "pc", "mod", "shared.xl");

            ArchiveXlSourceScanResult scan = ArchiveXlSourceScanner.Scan([new ArchiveXlProviderSource("High", high), new ArchiveXlProviderSource("Middle", middle), new ArchiveXlProviderSource("Low", low)], null, new HashSet<string>(StringComparer.OrdinalIgnoreCase) { disappeared });
    ...
    [TestMethod]
    public void SourceScanDoesNotPromoteTwoLosersWhenTheDeployedWinnerIsLocked()
    {
        string root = Path.Combine(Path.GetTempPath(), "conflict-studio-locked-archive-xl-winner-" + Guid.NewGuid().ToString("N"));
        string high = Path.Combine(root, "High");
        string middle = Path.Combine(root, "Middle");
        string low = Path.Combine(root, "Low");
        try
        {
            WriteManifest(high, "shared.xl", "factories:\n  - high\\factory.csv\n");
            WriteManifest(middle, "shared.xl", "factories:\n  - middle\\factory.csv\n");
            WriteManifest(low, "shared.xl", "factories:\n  - low\\factory.csv\n");
            WriteManifest(low, "other.xl", "factories:\n  - low\\other.csv\n");
            string lockedPath = Path.Combine(high, "archive", "pc", "mod", "shared.xl");
            Dictionary<string, string> winners = new(StringComparer.OrdinalIgnoreCase) { ["archive\\pc\\mod\\shared.xl"] = "high", ["archive\\pc\\mod\\other.xl"] = "low" };
            using FileStream locked = File.Open(lockedPath, FileMode.Open, FileAccess.ReadWrite, FileShare.None);

            ArchiveXlSourceScanResult scan = ArchiveXlSourceScanner.Scan([new ArchiveXlProviderSource("High", high, "high"), new ArchiveXlProviderSource("Middle", middle, "middle"), new ArchiveXlProviderSource("Low", low, "low")], winners);

            Assert.AreEqual("archive\\pc\\mod\\other.xl", scan.Sources.Single().FilePath);
            Assert.AreEqual("High", scan.Failures.Single().Provider);
        }
        finally
        {
            Directory.Delete(root, true);
