...
public sealed class DeepScriptEvidenceAnalyzerTests
{
    ...
    [TestMethod]
    public void RedScriptFlowDoesNotCarryAnAnnotationAcrossANewerMethodAnnotation()
    {
        RedScriptSource source = new("Alpha", "alpha.reds", "@wrapMethod(OldSystem)\n@replaceMethod(NewSystem)\npublic final func Update(\n  value: Int32\n) -> Void { }");

        RedScriptFlowEvidence[] evidence = RedScriptFlowEvidenceAnalyzer.Analyze([source]);

        Assert.AreEqual(1, evidence.Length);
        Assert.AreEqual("NewSystem.Update(Int32)", evidence.Single().Target);
        Assert.AreEqual(RedScriptFlowKind.Replace, evidence.Single().Kind);
    }

    [TestMethod]
    public void RedScriptFlowHashesOnlyTheAnnotatedDeclarationAndBody()
    {
        string method = "@wrapMethod(DamageSystem)\npublic final func ProcessHit(value: Int32) -> Void {\n  this.Record(value);\n  wrappedMethod(value);\n}";
        RedScriptSource original = new("Alpha", "alpha.reds", method + "\npublic func Unrelated() -> Void { this.First(); }");
        RedScriptSource unrelatedEdit = original with { Text = method + "\npublic func Unrelated() -> Void { this.Second(); }" };
        RedScriptSource lineShift = original with { Text = "\n\n" + method };
        RedScriptSource bodyChange = original with { Text = method.Replace("this.Record(value);", "this.Replace(value);", StringComparison.Ordinal) };

        RedScriptFlowEvidence first = RedScriptFlowEvidenceAnalyzer.Analyze([original]).Single();
        RedScriptFlowEvidence unrelated = RedScriptFlowEvidenceAnalyzer.Analyze([unrelatedEdit]).Single();
        RedScriptFlowEvidence shifted = RedScriptFlowEvidenceAnalyzer.Analyze([lineShift]).Single();
        RedScriptFlowEvidence changed = RedScriptFlowEvidenceAnalyzer.Analyze([bodyChange]).Single();

        Assert.IsNotNull(first.BodySha256);
        Assert.AreEqual(first.BodySha256, unrelated.BodySha256);
        Assert.AreEqual(first.BodySha256, shifted.BodySha256);
        Assert.AreNotEqual(first.BodySha256, changed.BodySha256);
    }
    ...
    [TestMethod]
    public void LuaEvidenceHashesOnlyTheRegisteredInlineCallback()
    {
        string registration = "Override('DamageSystem', 'ProcessHit', function(self, wrapped) self.Record(); return wrapped() end)";
        LuaSource original = new("Alpha", "alpha.lua", registration + "\nlocal unrelated = 'first'");
        LuaSource unrelatedEdit = original with { Text = registration + "\nlocal unrelated = 'second'" };
        LuaSource bodyChange = original with { Text = registration.Replace("self.Record()", "self.Replace()", StringComparison.Ordinal) };

        LuaCallbackEvidence first = LuaCallbackEvidenceAnalyzer.Analyze([original]).Single();
        LuaCallbackEvidence unrelated = LuaCallbackEvidenceAnalyzer.Analyze([unrelatedEdit]).Single();
        LuaCallbackEvidence changed = LuaCallbackEvidenceAnalyzer.Analyze([bodyChange]).Single();

        Assert.IsNotNull(first.CallbackSha256);
        Assert.AreEqual(first.CallbackSha256, unrelated.CallbackSha256);
        Assert.AreNotEqual(first.CallbackSha256, changed.CallbackSha256);
    }

    [TestMethod]
    public void LuaEvidenceResolvesNamedCallbackBodyPassedThroughAHelper()
    {
        LuaSource source = new("Alpha", "alpha.lua", "local function callback(self, wrapped) self.Record(); return wrapped() end\nlocal function registerOverride(className, methodName, callbackValue)\n  Override(className, methodName, callbackValue)\nend\nregisterOverride('DamageSystem', 'ProcessHit', callback)");
        LuaSource changed = source with { Text = source.Text.Replace("self.Record()", "self.Replace()", StringComparison.Ordinal) };

        LuaCallbackEvidence evidence = LuaCallbackEvidenceAnalyzer.Analyze([source]).Single(value => value.Confidence == EvidenceConfidence.Literal);
        LuaCallbackEvidence changedEvidence = LuaCallbackEvidenceAnalyzer.Analyze([changed]).Single(value => value.Confidence == EvidenceConfidence.Literal);

        Assert.AreEqual(LuaContinuationEvidence.Continues, evidence.Continuation);
        Assert.IsNotNull(evidence.CallbackSha256);
        Assert.AreNotEqual(evidence.CallbackSha256, changedEvidence.CallbackSha256);
    }
