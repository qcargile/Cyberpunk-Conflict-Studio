...
public sealed class EvidenceDecisionStoreTests
{
    ...
    [TestMethod]
    public void ConcurrentStoresPreserveEveryReviewedDecision()
    {
        string root = Path.Combine(Path.GetTempPath(), "conflict-studio-decisions-concurrent-" + Guid.NewGuid().ToString("N"));
        try
        {
            using ManualResetEventSlim start = new(false);
            Task[] reviews = Enumerable.Range(0, 16).Select(index => Task.Run(() =>
            {
                EvidenceDecisionStore store = new(root);
                ConflictWorkItem item = new(ConflictSurface.ScriptAndTweak, $"Target{index}", EvidenceClassification.Review, ConflictWorkState.ReviewWhenRelevant, "summary", "action", null, ["Alpha", "Beta"], index.ToString("x64", System.Globalization.CultureInfo.InvariantCulture));
                start.Wait();
                store.Review("install", "Standard", item, "review", DateTimeOffset.UtcNow);
            })).ToArray();

            start.Set();
            Task.WhenAll(reviews).GetAwaiter().GetResult();

            Assert.AreEqual(16, new EvidenceDecisionStore(root).Load().Length);
        }
        finally
        {
            if (Directory.Exists(root)) Directory.Delete(root, true);
