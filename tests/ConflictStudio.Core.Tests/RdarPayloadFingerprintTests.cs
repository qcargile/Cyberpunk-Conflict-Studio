...
public sealed class RdarPayloadFingerprintTests
{
    ...
    [TestMethod]
    public void ApplyContainsCookedDecodeFailureToOneResource()
    {
        string path = Path.Combine(Path.GetTempPath(), "conflict-studio-payload-contained-" + Guid.NewGuid().ToString("N") + ".archive");
        try
        {
            byte[] file = new byte[27];
            for (int offset = 0; offset < file.Length; offset += 9)
            {
                BinaryPrimitives.WriteUInt32LittleEndian(file.AsSpan(offset), 1263681867);
                BinaryPrimitives.WriteUInt32LittleEndian(file.AsSpan(offset + 4), 1);
                file[offset + 8] = (byte)(offset / 9 + 1);
            }
            File.WriteAllBytes(path, file);
            Dictionary<ulong, RdarResourceStorage> storage = new() { [1] = new(0, 1), [2] = new(1, 2), [3] = new(2, 3) };
            RdarArchivePayloadIndex index = new("Alpha.archive", path, storage, [new(0, 9, 1), new(9, 9, 1), new(18, 9, 1)]);
            ResourceProvider[] providers =
            [
                new("Alpha.archive", 1, "base\\one.mesh", null), new("Beta.archive", 1, "base\\one.mesh", null),
                new("Alpha.archive", 2, "base\\two.mesh", null), new("Beta.archive", 2, "base\\two.mesh", null),
                new("Alpha.archive", 3, "base\\three.mesh", null), new("Beta.archive", 3, "base\\three.mesh", null)
            ];

            ResourceProvider[] result = RdarPayloadFingerprint.ApplyWithDecoder(providers, [index], (compressed, _) => compressed[0] == 2 ? throw new InvalidDataException("bad cooked payload") : [compressed[0]]);

            Assert.IsNotNull(result.Single(value => value.ArchiveName == "Alpha.archive" && value.ResourceHash == 1).CookedPayloadSha256);
            Assert.IsNull(result.Single(value => value.ArchiveName == "Alpha.archive" && value.ResourceHash == 2).CookedPayloadSha256);
            Assert.IsNotNull(result.Single(value => value.ArchiveName == "Alpha.archive" && value.ResourceHash == 3).CookedPayloadSha256);
        }
        finally
        {
            File.Delete(path);
