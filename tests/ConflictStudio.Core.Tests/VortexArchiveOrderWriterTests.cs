...
public sealed class VortexArchiveOrderWriterTests
{
    ...
    public void ApplySendsAnExactProfileBoundRequestAndReturnsBridgeVerification()
    {
        ...
        Assert.AreEqual(TimeSpan.FromSeconds(60), captured.ExpiresAtUtc - captured.RequestedAtUtc);
    ...
    [TestMethod]
    public void ApplyDoesNotUseDesktopContextAgeAsWriteAuthorization()
    {
        ArchiveOrderObservation observation = new("Standard", "C:\\Vortex", null, Inventory, CurrentOrder);
        ArchiveOrderPreview preview = ArchiveOrderPlanner.CreatePreview(observation, ProposedOrder);
    ...
    [TestMethod]
    public void RequestStoreRejectsASecondConcurrentApplicationInsteadOfOverwritingThePendingRequest()
    {
        string root = Path.Combine(Path.GetTempPath(), "conflict-studio-vortex-concurrent-" + Guid.NewGuid().ToString("N"));
        try
        {
            VortexOrderBridgeStore first = new(root);
            VortexOrderBridgeStore second = new(root);
            DateTimeOffset now = DateTimeOffset.UtcNow;
            VortexOrderRequest firstRequest = new(1, new string('e', 32), new string('a', 64), "profile", now, now.AddSeconds(15), null, Inventory, ProposedOrder);
            VortexOrderRequest secondRequest = firstRequest with { RequestId = new string('f', 32) };
            using CancellationTokenSource cancellation = new();
            Task firstExchange = Task.Run(() => Assert.ThrowsExactly<OperationCanceledException>(() => first.Exchange(firstRequest, TimeSpan.FromSeconds(15), cancellation.Token)));
            Assert.IsTrue(SpinWait.SpinUntil(() => first.ReadRequest() is not null, TimeSpan.FromSeconds(2)));

            ArchiveOrderException exception = Assert.ThrowsExactly<ArchiveOrderException>(() => second.Exchange(secondRequest, TimeSpan.FromSeconds(1)));

            StringAssert.Contains(exception.Message, "already waiting");
            Assert.AreEqual(firstRequest.RequestId, first.ReadRequest()!.RequestId);
            cancellation.Cancel();
            Assert.IsTrue(firstExchange.Wait(TimeSpan.FromSeconds(2)));
        }
        finally
        {
            Directory.Delete(root, true);
