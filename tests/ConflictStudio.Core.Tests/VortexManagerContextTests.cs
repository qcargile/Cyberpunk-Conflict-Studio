...
{
    [TestMethod]
    public void CompleteDeploymentMapTreatsAbsentPathsAsGameFilesOnly()
    {
        Dictionary<string, string> winners = new(StringComparer.OrdinalIgnoreCase);

        Assert.IsFalse(VortexDeploymentFiles.IsEffective("alpha", "r6\\scripts\\ghost.reds", winners));
        Assert.IsTrue(VortexDeploymentFiles.IsEffective("game-directory", "r6\\scripts\\base.reds", winners));
        Assert.IsTrue(VortexDeploymentFiles.IsEffective("alpha", "r6\\scripts\\ghost.reds", null));
    }
    [TestMethod]
    ...
    public void ReadAcceptsDuplicateArchiveEntriesForRepair()
    {
        ...
        try
        {
            ...
            VortexManagerContext changed = Context(game, staging, [], new Dictionary<string, string>()) with { ArchiveOrder = ["Alpha.archive", "Alpha.archive"] };
            VortexManagerContext context = changed with { ContextId = VortexManagerContextStore.ComputeContextId(changed) };
            string path = Path.Combine(root, "context.json");
    ...
    [TestMethod]
    public void ReadKeepsAMissingProviderForScanDiagnostics()
    {
        string root = Path.Combine(Path.GetTempPath(), "conflict-studio-vortex-missing-provider-" + Guid.NewGuid().ToString("N"));
        try
        {
            string game = Path.Combine(root, "game");
            string staging = Path.Combine(root, "staging");
            ...
            Directory.CreateDirectory(game);
            Directory.CreateDirectory(staging);
            VortexManagerContext context = Context(game, staging, [new("missing", "Missing provider", missing, 0)], new Dictionary<string, string>());
            string path = Path.Combine(root, "context.json");
            File.WriteAllText(path, JsonSerializer.Serialize(context));

            VortexManagerContext actual = VortexManagerContextStore.Read(path);

            Assert.AreEqual(missing, actual.Providers.Single().RootPath);
        }
        finally
        {
            Directory.Delete(root, true);
    ...
    [TestMethod]
    public void ReadDisambiguatesDuplicateProviderNamesWithTheirIds()
    {
        ...
        try
        {
            ...
            Assert.AreEqual("Same name [alpha]", actual.Providers[0].Name);
            Assert.AreEqual("Same name [beta]", actual.Providers[1].Name);
        }
        finally
        {
            Directory.Delete(root, true);
    ...
    public void Mo2GuardRejectsAStillDeployedVortexProfileInTheSameGameRoot()
    {
        ...
        try
        {
            ...
            string source = Path.Combine(provider, "r6", "scripts", "shared.reds");
            ...
            File.WriteAllText(source, "deployed");
            File.WriteAllText(deployed, "deployed");
            VortexManagerContext changed = Context(game, staging, [new("alpha", "Alpha", provider, 0)], new Dictionary<string, string> { ["r6\\scripts\\shared.reds"] = "alpha", ["r6\\scripts\\absent.reds"] = "alpha" }) with { ProfileName = "Vortex Default" };
            VortexManagerContext context = changed with { ContextId = VortexManagerContextStore.ComputeContextId(changed) };
    ...
    [TestMethod]
    public void Mo2GuardDoesNotDemandPurgeForAStaleWinnerPathOwnedByDifferentBytes()
    {
        string root = Path.Combine(Path.GetTempPath(), "conflict-studio-vortex-stale-winner-" + Guid.NewGuid().ToString("N"));
        try
        {
            string game = Path.Combine(root, "game");
            string staging = Path.Combine(root, "staging");
            ...
            Directory.CreateDirectory(Path.GetDirectoryName(source)!);
            Directory.CreateDirectory(Path.GetDirectoryName(deployed)!);
            File.WriteAllText(source, "old-vortex-bytes");
            File.WriteAllText(deployed, "current-mo2-bytes");
            VortexManagerContext context = Context(game, staging, [new("alpha", "Alpha", provider, 0)], new Dictionary<string, string> { ["r6\\scripts\\shared.reds"] = "alpha" });
            string contextPath = Path.Combine(root, "context.json");
            File.WriteAllText(contextPath, JsonSerializer.Serialize(context));
    ...
    [TestMethod]
    public void Mo2GuardIgnoresACorruptContextThatCannotBeMatchedToTheGame()
    {
        string root = Path.Combine(Path.GetTempPath(), "conflict-studio-vortex-corrupt-owner-" + Guid.NewGuid().ToString("N"));
        try
        {
            string game = Path.Combine(root, "game");
            Directory.CreateDirectory(game);
            string contextPath = Path.Combine(root, "context.json");
            File.WriteAllText(contextPath, "invalid");
    ...
    [TestMethod]
    public void Mo2GuardBlocksACorruptContextThatStillNamesTheSameGame()
    {
        string root = Path.Combine(Path.GetTempPath(), "conflict-studio-vortex-corrupt-matching-owner-" + Guid.NewGuid().ToString("N"));
        try
        {
            string game = Path.Combine(root, "game");
            Directory.CreateDirectory(game);
            string contextPath = Path.Combine(root, "context.json");
            File.WriteAllText(contextPath, JsonSerializer.Serialize(new { gameRoot = game, schemaVersion = "bad" }));
    ...
    [TestMethod]
    public void Mo2GuardBlocksAMatchingWinnerWhoseStagedSourceIsMissing()
    {
        string root = Path.Combine(Path.GetTempPath(), "conflict-studio-vortex-missing-staged-owner-" + Guid.NewGuid().ToString("N"));
        try
        {
            string game = Path.Combine(root, "game");
            string staging = Path.Combine(root, "staging");
    ...
    {
        VortexManagerContext context = new(1, string.Empty, new DateTimeOffset(2026, 8, 29, 18, 0, 0, TimeSpan.Zero), "profile-1", "Standard", game, staging, true, providers, new Dictionary<string, string>(winners, StringComparer.OrdinalIgnoreCase), ["Alpha.archive", "Beta.archive"], null);
